	...
	protogen.Options{ParamFunc: flagSet.Set}.Run(
		func(plugin *protogen.Plugin) error {
			...
			for _, file := range plugin.Files {
				if file.Generate {
					generateFile(plugin, file, *packageSuffix)
				}
			}
...
func generateFile(plugin *protogen.Plugin, file *protogen.File, packageSuffix string) {
	...
	generatedFile := plugin.NewGeneratedFile(
		file.GeneratedFilenamePrefix+filenameSuffix,
		goImportPath,
	)
	if packageSuffix != "" {
